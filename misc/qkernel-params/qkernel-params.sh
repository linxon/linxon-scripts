...
export \
	__HOME_DIR__="${QKERNEL_HOME_DIR:-/tmp/$(basename $0)}"
	__CEC_STATE_FILE__="${__HOME_DIR__}/.check_extra_config_state" \
...
do
	...
	pushd "${__HOME_DIR__}" >/dev/null && (
		...
		__dis_func__ 'src_unpack src_prepare move_old_moduledb remove_moduledb
			...
			debug-print-section tc-ld-disable-gold tc-export linux-mod_pkg_setup'
